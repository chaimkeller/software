...
MouseMove_Error:
   ...
   'MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure MouseMove of User Control FlexListBox"
End Sub
...
UserControl_KeyDown_Error:
   ...
   'MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure UserControl_KeyDown of User Control FlexListBox"
End Sub
...
Public Sub AddItem(ByVal sListItem As String, Optional sKey As String = "", _
    Optional bEnabeled As Boolean = True, Optional lItemForeColor As Long, _
    Optional lItemBackColor As Long, Optional vUnderlayingValue As Variant = "", _
    Optional sToolTipText As String = "", Optional oPicture As Picture, _
    Optional oDownPicture As Picture, _
    Optional bVisible As Boolean = True) '<<<change
Dim l As Long
...
With List
   ...
   .item(1).ItemForeColor = lItemForeColor '<<<change
   .item(1).ItemBackColor = lItemBackColor '<<<change
...
AddItem_Error:
   ...
   'MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure AddItem of User Control FlexListBox"
...
For l = IIf(lItem > -1, lItem, lTopIndex + 1) To IIf(lItem > -1, lItem, List.Count) Step 1
   With List.item(l)
      If .Visible Then
         ...
         'allow for different forecolors for each item <<<change
         If .ItemForeColor <> 0 Then lblItems(i).ForeColor = .ItemForeColor '<<<change
         'allow for different backcolors for each item <<<change
         If .ItemBackColor <> 0 Then lblItems(i).BackColor = .ItemBackColor '<<<change
