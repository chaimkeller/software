...
There are three major table types with subdivisions
1) "BY": visible, mishor, astronomical tables for the yeshuvim of Eretz Yisroel
	the data used for the calculations reside in the "cities" directory, each yeshuv has its own subdirectory 
	that is named using the English city name.  Hebrew names are retrieved from certain text files (see code)
	further subdivided into netz and skiy directories containing a bat file and
	indvidual profile files generated by the rdhalba4 program using the 25 m JKH DTM of EY
	The files that are used for each calculation are listed in the "bat" file
	The bat file contains a list of profile name, ITMx, ITMy, height (m)
2) "Chai" visible, mishor, astronomical tables for cities outside of Eretz Yisroel, also for EY neighborhoods
	files reside in the cities\eros directory, each city has its own subdirectory called by its name,
	further subdivided into netz and skiy subdirectories containing the "bat" file and the individual 
	profile files.  Only files within the bat file can be searched to check if they fall within the
	chosen search radius. Only profiles within the search radius around the chosen center and
	used for the calculations..
3) "Astr" mishor, astronomical times for any coordinates, visible times for any coordinates
		  including Golden Hour
    For visible time, a profile is generated from the DTM tiles found in the "USA" directory.
	The profile generation is identical to the one used in newreadDTM.exe for generating
	profiles for the Maps and More program

Refraction used in this program was determined by ray tracing, the old version used Menat's
the new one uses van der Werf's (VDW).  VDW refraction is calculated for the standard temperature
and pressure for the points height and then modified for the predicted ground temperature using
VDW scaling rules.  Ground temperatures are based on the WorldClim 2 1 arcsec data for 1990-2020.
Those files resdie in the WorldClim_bil directory.  They were converted from the original geotif
format to 2 byte bil files using Global Mapper 13.  In the future, those files will need to be
updated.whenever the WorldClim 1 arcsec data is updated

The individual functions used by this program are too numerous to summarize their use.
Please read the brief documentation for each routine

This program is best understood by stepping through it using a debugger, such as the Microsoft VC6 debugger,
or the debugger that is built into DevCpp version 5.and stepping through th esteps.
To run the program in debug mode convert the line
if (argc != 26) // && argc != 1 ) to if (argc != 26 && argc != 1 )
Then comment out the /* for the next logic block that has the test values.
The values set for debugging the program are simply the cgi arguments that are passed to main as arguments via the cgi, i.e.,
1. Table Type (BY,Chai,Astr)
2. MetroArea (for BY,Chai tables0
3. g_yrheb = Hebrew year in digits, e.g. 5781
4. zmanyes = 1 to calculate z'manay hayom along with the individual tables for sunrise/sunset
5. typeman which zmaneyei hayom opinion
6. optionheb = true/false for Hebrew/English
7. RoundSecondsin = round to RoundSecondsin seconds
8: numUSAcities1 = metro area number (based on the alphabetical list that appears in the dir files in the cities directories)
9: numUSAcities2 = neighborhood within city area number
10 searchradius = used for Chai type tables to set the search radius in kilometers
11 yesmetro = 0/1 used for Chai type tables: sets flag to use coordinates of center of search instead of stored coordinates of any neighborhood
12 GoldenLight = 0/1/2 program flag for Golden Light sunrise/sunset (not part of the cgi parameters)
13 eroslatitudestr = latitude of place (used for Chai and Astr tables) string variable
14 eroslongitudestr = longitude, etc.
15 eroshgtstr = height of place as string
...
///////////////////////////////////////////////////////////////////////////
{
	...
	while( ! feof(stream) )
	{
		...
		if ( dist <= searchradius )
		{
			...
			//so for more accurate accounting for this for calculating z'manei hayom, then set FixProfHgtBug to true
			...
			if (FixProfHgtBug) {
				...
				fillen = strlen(citnam);
    			if ( !(stream2 = fopen( citnam, "r" )) )
    			{
				 strlwr(citnam + fillen - 12); //try with all lowercases
    				 if ( !(stream2 = fopen( citnam, "r" )) )
    				 {
    					 strupr(citnam + fillen - 12); //try with all uppercase
    					 if ( !(stream2 = fopen( citnam, "r" )) )
    					 {
    	         			goto cn500; //can't open profile file for some reason, ignore error and skip try to find profile's hgt value
    					 }
    				 }
    			}
				...
				{
					hgt0 = hgt; //store bat file height
					hgt = atof( &strarr[2][0] ); //hgt is the third data ןאקצ in this doc line separated by commas

					if (hgt0 == hgt) hgt += hgtobs; //bat and profile heights were equal, almost certain
														//indication that the ground height was recorded instead
														//of the calculation height = hgt + hgtobs
				}
				else
				{
...
//	as well as all uppercases and all lowercaes
	...
	//101520: following code is depricated; height checks is now handled in routines netzskiy and calnearsearch
	/*  
	//parse it to obtain height recorded in profile file which is more reliable than the bat file
	if (!ParseString( doclin, ",", strarr, MAXARRSIZE ) )
	{
...
/* 				remove old terrestrial refraction values from the view angle */
	...
	while ( !feof(stream) )
	{
		...
		{
			doclin[0] = 0;
			fgets_CR( doclin, 255, stream ); //read next line of data
			if (!ParseString( doclin, ",", strarr, MAXARRSIZE ) )
			{
				tmp1 = atof( &strarr[0][0] ); //azimuth
				tmp2 = atof( &strarr[1][0] ); //view angle
...
L550:
	...
	if (!CGI || Windows)
	{
		if ( f =fopen("C:/JK_C/EROS.TMP", "w" ) )
		{
			...
			////////////////////////////fixed on 101520/////////////record actual profile height with added observer height = hgt and not hgt0 ///
			fprintf( f, "%lg,%lg,%lg,%lg,%lg,%lg,%lg,%lg\n", lat0, -lon0, hgt, -startkmx, -sofkmx, dstpointx, dstpointy, apprnr); //longitude sign convention
		}
	}
