...
Public CHMAP(14, 26) As String * 2, kmxoo, kmyoo, filnumg%, hgt, hgtpos
Public CHMNE As String * 2, CHMNEO As String * 2, SF As String * 2, picf$
...
Public maxang%, fullrange%, diflat%, diflog%, maxangf%, viewmode%, fullrangef%, diflatf%, diflogf%
...
Public israeldtm As String * 1, israeldtmcd As Boolean, topotype%, AutoNum&
Public israeldtmf As String * 1, israeldtmcdf As Boolean, mapSearchVis As Boolean
Public worlddtm As String * 1, worlddtmcd As Boolean, ExplorerDir As String
...
Public adx1, bdy1, adx1f, bdy1f, drivjk$, drivfordtm$, drivprom$, drivprof$, drivcities$, drivdtm$
...
Public FileViewFileName() As String 'dynamic array
Public FileViewDir$, FileViewFileType(20) As Integer, UniqueRoots%, fileo$, setflag%, coordAnalyze(2) As Double
Public OutFile$, bAirPath As Boolean, kmxTrig, kmyTrig, hgtTrig, DTMflag As Integer
Public SearchVis As Boolean, WinVer As Long, xdim As Double, ydim As Double
...
Public Sub worldheights(lg, lt, hgt)
   ...
   If DTMflag > 0 And (lt >= -60 And lt <= 61) Then 'SRTM
       ...
       'now load the buffers
        For i% = 1 To 9
           ...
           Else
              ...
              End If
           If n50x%(i%) >= 4 Then
              picf$ = "d:\turbo2cd\Itmv2s\MAP50\CLI" + pic1$(i%) + "." + pic2(i%)
              If picold$(i%) = picf$ Then GoTo ld200
              myfile = Dir(picf$)
              If myfile <> sEmpty Then
                 Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
              Else 'load blank bitmap
                 picf$ = "d:\turbo2cd\Itmv2s\MAP50\CLI0707.dl"
                 If picold$(i%) = picf$ Then GoTo ld200
                 Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
                 End If
              picold$(i%) = picf$
           Else
              picf$ = "d:\turbo2cd\Itmv2s\MAP50\CLI0" + pic1$(i%) + "." + pic2(i%)
              If picold$(i%) = picf$ Then GoTo ld200
              myfile = Dir(picf$)
              If myfile <> sEmpty Then
                 Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
...
ld200:
     ...
     ElseIf topotype% = 1 Then 'new 50000 maps
       ...
       'now load the buffers
        For i% = 1 To 9
            picf$ = "d:\turbo2cd\Itmv3.1\map50\cli" + LTrim$(pic1$(i%) & ".bmp")
            If picold$(i%) = picf$ Then GoTo ld300
            myfile = Dir(picf$)
            If myfile <> sEmpty Then
               Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
            Else 'load blank bitmap
               picf$ = "d:\turbo2cd\Itmv3.1\map50\cli0707.bmp"
               If picold$(i%) = picf$ Then GoTo ld300
               Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
               End If
           picold$(i%) = picf$
...
ElseIf map400 = True Then
   ...
   For i% = 1 To 9
       ...
       If n400x%(i%) >= 2 Then
          picf$ = "d:\turbo2cd\Itmv2s\MAP400\C4u" + pic1$(i%) + "." + pic2(i%)
          ...
          picold$(i%) = picf$
       Else
          picf$ = "d:\turbo2cd\Itmv2s\MAP400\C4u0" + pic1$(i%) + "." + pic2(i%)
          If picold$(i%) = picf$ Then GoTo ld400
          myfile = Dir(picf$)
          If myfile <> sEmpty Then
             Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
...
ElseIf world = True Then
   ...
   For i% = 1 To 9
       ...
       If mapimport = False Then
            picf$ = "d:\turbo2cd\worldl" + pic1$(i%) + "." + pic2(i%)
            If picold$(i%) = picf$ Then GoTo ld600
            myfile = Dir(picf$)
            If myfile <> sEmpty Then
               Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
            Else 'load blank bitmap
               picf$ = "d:\turbo2cd\worldl33.ur"
               If picold$(i%) = picf$ Then GoTo ld600
               Maps.PictureClip1(i% - 1).Picture = LoadPicture(picf$)
               End If
            picold$(i%) = picf$
...
'           End If
        ...
        If (Maps.Text5.Text < 70000 Or Maps.Text5.Text > 240000 Or Maps.Text6.Text < 870000 Or Maps.Text6.Text > 1310000) And Maps.Label5.Caption = "ITMx" Then
           If (Maps.Text5.Text < 250 And Maps.Text5.Text > 80) And (Maps.Text6.Text < 280 And Maps.Text6.Text > -125) Then '>80 -- old limit
...
Public Sub obstructions(Dest)
  ...
  If printing = False Then
     mult% = 2 'mag '*****allows to printscreen until debug
...
Public Sub sunrisesunset(mode%)
    Dim xdim As Double, ydim As Double, skip As Boolean
    Dim skipkmx As Double, skipkmy As Double, lt As Double, lg As Double
    Dim beglog As Double, endlog As Double, kmx As Double
    Dim beglat As Double, endlat As Double, kmy As Double
...
skipcheck:
    ...
    If myfile <> sEmpty Then
       ...
       If filn33$ = filn3$ + ".BIN" And modch% = sunmode% And modval% = modvalch% And myfile2 <> sEmpty And _
          ...
          beglat = beglatch
          endlatch = endlatch
          Open drivjk$ + "eros.tm3" For Output As #filtmp%
          Print #filtmp%, filn33$
...
80    doclin$ = Dir("c:\dtm\*.BIN")
      ...
      If doclin$ <> sEmpty And myfile <> sEmpty And Dir("c:\dtm\*.BI1") <> sEmpty Then 'read this eros.tm3 and check it
         ...
         If ramdrive + ":\" + doclin$ = filn3$ + ".BIN" And modch% = sunmode% And modval% = modvalch% And _
            ...
            If myfile <> sEmpty Then Kill drivjk$ + "eros.tmp"
...
'     Print #filtmp%, Format(beglog, "##0.00000"), Format(endlog, "##0.00000"), Format(beglat, "#0.00000"), Format(endlat, "#0.00000")
     ...
     myfile = Dir(drivjk$ + "newreaddtm.end") 'delete old end flag file if exists
     If myfile <> sEmpty Then Kill drivjk$ + "newreaddtm.end"
...
tst2:  myfile = Dir(ramdrive + ":\*.BI1")
       ...
       With mapprogressfm
        ...
        .Text1 = apprn
       End With
       If skip = True Then mapprogressfm.Text1 = apch
       ret = SetWindowPos(mapprogressfm.hWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE + SWP_NOMOVE)
       ...
       If viewer3D = True Then
          ...
          'If DTMflag > 0 Then
             ...
             'flag newreadDTM to write land.x file
             '(and not land1.x)
             Write #filtmp4%, 0
             Close #filtmp4%
             'End If
       ...
       If accept = False Then
           ...
           GoTo mm999
          End If
       If skip = True And apprn = apch And Dir(drivjk$ + "eros.tmp") <> sEmpty Then
          GoTo mm500
          End If
       Screen.MousePointer = vbHourglass
       filtm3num% = FreeFile
       Open drivjk$ + "eros.tm3" For Output As #filtm3num%
...
'*********************************end of C++ rderos2 program***************
       ...
       myfile = Dir(drivjk$ + "erosend.tmp")
       If myfile <> sEmpty Then Kill drivjk$ + "erosend.tmp"
       myfile = Dir(drivjk$ + "erostat.tmp")
       If myfile <> sEmpty Then Kill drivjk$ + "erostat.tmp"
...
'    run C++ program that extracts the relevant parts of the DTMs
    ...
    'don't go on until finished computing
ts5: endflag% = 0
     myfile = Dir(drivjk$ + "newreaddtm.end")
     If myfile = sEmpty Then
        waittim = Timer + 0.2
        Do Until Timer > waittim
           DoEvents
     ...
     Else
        endfil% = FreeFile
        Open drivjk$ + "newreaddtm.end" For Input As #endfil%
        Input #endfil%, endflag%
        Close #endfil%
        End If
     'do second check using Windows API
     ...
     'if not running egg, then run rderos2
      ...
      RetVal = Shell(drivjk$ + "rderos2.exe", 2) ' Run rderos2 as DOS shell
      ...
      Do Until Timer > waitime + waiting%
         DoEvents
      Loop
      lResult = FindWindow(vbNullString, "rderos2")
      If lResult <> 0 Then
          mapprogressfm.Visible = True
          mapEROSDTMwarn.Visible = False
          ret = SetWindowPos(mapprogressfm.hWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE + SWP_NOMOVE)
          ...
          Do Until Timer > waitime + 0.5
             DoEvents
          Loop
          'now plot the eros.tmp file output from rderos2
          myfile = Dir(drivjk$ + "erostat.tmp")
          If myfile <> sEmpty Then
suns500:     tmpfil% = FreeFile
             Open drivjk$ + "erostat.tmp" For Input As #tmpfil%
             nlines% = 0
             Do Until EOF(tmpfil%)
...
mm450: Do Until lResult = 0
          If Dir(drivjk$ + "erosend.tmp") <> sEmpty Then
             nstat% = 100
          ...
          ret = SetWindowPos(mapprogressfm.hWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE + SWP_NOMOVE)
          myfile = Dir(drivjk$ + Trim$(Str$(nstat%)))
          tmpfil% = FreeFile
          If myfile <> sEmpty Then
             Open drivjk$ + LTrim$(Str(nstat%)) For Input As #tmpfil%
             nlines% = 0
             Do Until EOF(tmpfil%)
                Input #tmpfil%, nstat%
                nlines% = 1
       ...
       Do Until Timer > waitime
          DoEvents
       Loop
       If Dir(drivjk$ + "erostat.tmp") <> sEmpty Then Kill drivjk$ + "erostat.tmp"
       If Dir(drivjk$ + "erosend.tmp") <> sEmpty Then Kill drivjk$ + "erosend.tmp"
       For i% = 1 To maxstat%
          If Dir(drivjk$ + Trim$(Str$(i%))) <> sEmpty Then Kill drivjk$ + Trim$(Str$(i%))
       Next i%

       waitime = Timer
       Do While Dir(drivjk$ + "eros.tmp") = sEmpty And Timer <= waitime + 1
          DoEvents
       Loop
       Screen.MousePointer = vbDefault
       If Dir(drivjk$ + "eros.tmp") = sEmpty Then 'if empty then, something is wrong
          MsgBox "Something wrong with the rderso2 analysis." & vbLf & _
...
diskerrhandler:
   ...
   myfile = Dir(drivjk$ + "eros.tm3")
   If myfile <> sEmpty Then Kill drivjk$ + "eros.tm3"
   myfile = Dir(ramdrive + ":\*.bin")
   If myfile <> sEmpty Then Kill ramdrive + ":\" + myfile
   myfile = Dir(ramdrive + ":\*.bi1")
   If myfile <> sEmpty Then Kill ramdrive + ":\" + myfile
...
Sub mapCrossSections()
   ...
   Else
    ...
    filtmp% = FreeFile
    Open drivjk$ + "crossect.tmp" For Output As #filtmp%
...
Sub EYsunrisesunset(mode%)
  ...
  'add the current place to the placlist.txt file
   ...
   FileCopy drivjk$ + "placlist.txt", drivjk$ + "viewin.tmp"
   FileViewName = drivjk$ + "placlist.txt"
   ...
   If Not FileViewError And FileEdit Then
      ...
      If Dir(drivjk$ + "placlist.bak") <> sEmpty Then
         filbak% = FreeFile
         Open drivjk$ + "placlist.bak" For Append As #filbak%
         filplac% = FreeFile
         Open drivjk$ + "placlist.txt" For Input As #filplac%
      ...
      FileCopy drivjk$ + "viewout.tmp", drivjk$ + "placlist.txt"
   Else
   ...
   Else
      ...
      End If
   filin% = FreeFile
   Open drivjk$ & "placlist.txt" For Input As #filin%
   filout% = FreeFile
   Open drivjk$ & "placlis2.txt" For Output As #filout%
   nsuffix% = 1
   uniqroot$ = sEmpty
   UniqueRoots% = 0
...
f50:  If Mid$(doc1$, 2, 8) <> uniqroot$ Then
      ...
      Else
         MsgBox "More than 99 profiles in directory!--abort", vbCritical + vbOKOnly, "Maps&More"
         Close #filin%
         Close #filout%
         Exit Sub
         End If
   ...
   If Dir(drivjk$ & "scanlist.txt") <> sEmpty Then Kill drivjk$ & "scanlist.txt"
   'Then run readlst3.exe
   ret = Shell(drivjk$ + "Readlst3.exe", vbNormalFocus)
   ...
   FileCopy drivjk$ + "scanlist.txt", drivjk$ + "viewin.tmp"
   FileViewName = drivjk$ + "scanlist.txt"
   mapFileViewfm.Visible = True
   ...
   'now dump the edited scanlist to scanlist.txt and
   'start rdhal2
   If Not FileEdit Then GoTo rhal 'no editing requested
   filtmp% = FreeFile
   Open drivjk$ & "viewout.tmp" For Input As #filtmp%
   Line Input #filtmp%, docline$
   Close #filtmp%
   filtmp% = FreeFile
   Open drivjk$ & "scanlist.txt" For Output As #filtmp%
   ...
   For i% = 1 To Len(docline$)
       ...
       Else
          ...
          End If
   Next i%
   Print #filtmp%, doclin$ 'print last line
   Close #filtmp%
   GoTo rhal
...
rhal:
   ...
   If response = vbYes Then
      If WinVer = 5 Or WinVer = 261 Then
         ...
         ret = WinExec(drivjk$ & "rdhalba4.exe", SW_SHOWNORMAL)
      Else
         ...
         Select Case response
            Case vbYes
               ret = Shell(drivjk$ & "rdhal2.bat", vbNormalFocus)
            Case vbNo
               ret = WinExec(drivjk$ & "rdhalba4.exe", SW_SHOWNORMAL)
            Case Else
               Close
